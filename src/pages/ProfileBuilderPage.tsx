...
export function ProfileBuilderPage() {
  ...
  return (
    <div className="flex-1 w-full bg-slate-50 relative">
      ...
      <div className="relative z-10 mx-auto max-w-2xl px-4 sm:px-6 pt-8 sm:pt-12 lg:pt-16">
        
        {/* Header & Progress */}
        <div className="mb-8 sm:mb-10 text-center">
           <span className="inline-flex items-center rounded-full bg-white/10 px-3 py-1 font-medium text-primary-200 border border-white/20 mb-4 backdrop-blur-md text-xs sm:text-sm">
             <Globe2 className="mr-1.5 h-3 w-3 sm:h-4 sm:w-4" /> Global Admissions
           </span>
           <h1 className="text-2xl sm:text-4xl font-extrabold text-white font-heading tracking-tight mb-8">
             Build Your Profile
           </h1>
            ...
            <Card className="border-0 shadow-2xl shadow-slate-200/60 rounded-3xl overflow-hidden bg-white/95 backdrop-blur-xl">
              <form onSubmit={handleSubmit}>
                ...
                <CardHeader className="px-5 sm:px-8 pt-6 sm:pt-8 pb-4">
                  <CardTitle className="text-xl sm:text-3xl font-bold font-heading text-slate-800">
                    {step === 1 && "Let's get to know you"}
                    {step === 2 && "Academic Background"}
                    {step === 3 && "English Proficiency"}
                    {step === 4 && "Preferences & Budget"}
                  </CardTitle>
                  <CardDescription className="text-sm sm:text-base text-slate-500 mt-2">
                ...
                <CardContent className="px-5 sm:px-8 pb-6 sm:pb-8 space-y-5 sm:space-y-8">
                  {step === 1 && (
                    <div className="space-y-5 sm:space-y-6">
                      <div>
                        ...
                        <Input 
                          ...
                          className="h-14 bg-slate-50 border-slate-200 text-lg rounded-2xl focus:bg-white transition-colors"
                        />
                      </div>
                      <div>
                        <label className="mb-3 block text-sm font-bold text-slate-700">Latest Education Level</label>
                      ...
                      <div>
                        ...
                        {profile.educationLevel !== 'A-Levels' ? (
                          <Input 
                            ...
                            className="h-14 bg-slate-50 border-slate-200 text-lg rounded-2xl focus:bg-white"
                          />
                        ) : (
                          <Input placeholder="e.g. A*AA" className="h-14 bg-slate-50 border-slate-200 text-lg rounded-2xl focus:bg-white" />
                        )}
                      </div>
                      <div>
                        <label className="mb-2 block text-sm font-bold text-slate-700">Intended Major / Program</label>
                        <Input 
                          required 
                  ...
                  {step === 3 && (
                    <div className="space-y-6">
                      <div>
                        <label className="mb-3 block text-sm font-bold text-slate-700">Have you taken an English Test?</label>
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                          {['IELTS', 'TOEFL', 'Duolingo', 'None'].map((test) => (
                            ...
                            >
                              <span className="font-bold text-sm tracking-wide">{test}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <AnimatePresence>
                        {profile.englishTest !== 'None' && (
                          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="pt-2">
                            <label className="mb-2 block text-sm font-bold text-slate-700">Overall Score</label>
                            <Input 
                              required 
                              type="number" 
                              step="0.5" 
                              ...
                              className="h-14 bg-slate-50 border-slate-200 text-lg rounded-2xl focus:bg-white w-full sm:w-1/2"
                            />
                          </motion.div>
                        )}
                        {profile.englishTest === 'None' && (
                          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
                            <p className="text-sm font-medium text-amber-800 bg-amber-50 border border-amber-200 p-4 rounded-2xl mt-4">
                  ...
                  {step === 4 && (
                    <div className="space-y-8">
                      <div>
                        <label className="mb-2 block text-sm font-bold text-slate-700">Yearly Budget (in BDT)</label>
                        <Input 
                          required 
                          type="number" 
                          ...
                          className="h-14 bg-slate-50 border-slate-200 text-lg font-semibold rounded-2xl focus:bg-white"
                        />
                      </div>
                      
                      <div>
                        <label className="mb-3 block text-sm font-bold text-slate-700">Preferred Destinations</label>
                          ...
                          {destinations.map(dest => {
                            ...
                            return (
                              <div
                                ...
                                className={cn(
                                  "cursor-pointer rounded-full border-2 px-5 py-2.5 text-sm font-bold transition-all",
                                  isSelected
                                    ? "border-primary-600 bg-primary-600 text-white shadow-md shadow-primary-600/30 transform scale-[1.02]"
                                    : "border-slate-200 bg-white text-slate-600 hover:border-slate-300 hover:bg-slate-50"
                                )}
                              >
                                {isSelected && <Check className="inline-block w-4 h-4 mr-1.5 -mt-0.5" />}
                                {dest}
                              </div>
                            );
                          })}
                      ...
                      <div className="flex items-start gap-4 rounded-2xl border-2 border-primary-100 bg-primary-50 p-5 cursor-pointer hover:border-primary-300 transition-colors" onClick={() => updateProfile({ needsFunding: !profile.needsFunding })}>
                        <div className="relative flex items-center mt-0.5">
                          <input
                            type="checkbox"
                            checked={profile.needsFunding}
                            onChange={(e) => updateProfile({ needsFunding: e.target.checked })}
                            className="peer h-6 w-6 cursor-pointer appearance-none rounded-md border-2 border-primary-300 checked:border-primary-600 checked:bg-primary-600 transition-all bg-white"
                          />
                          <svg className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 h-4 w-4 pointer-events-none opacity-0 peer-checked:opacity-100 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                          </svg>
                        </div>
                        <div>
                          <label className="cursor-pointer font-bold text-primary-900 select-none block">
                            I am looking for Scholarships / Financial Aid
                          </label>
                          <p className="text-sm text-primary-700 mt-1">We'll prioritize universities with high acceptance rates for funded programs.</p>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="pt-8 border-t border-slate-100">
                    <Button type="submit" size="lg" className="w-full text-lg h-14 rounded-2xl group relative overflow-hidden shadow-xl shadow-primary-500/20">
                      <span className="relative z-10 flex items-center">
                        {step === 4 ? "Find My Matches" : "Continue to Next Step"}
                        {step < 4 && <ChevronRight className="ml-2 h-5 w-5 group-hover:translate-x-1 transition-transform" />}
                      </span>
